...
NPC 模型请求会结合人格与该 NPC 自己的临时情绪状态。这是实验性的表演输入，不是身份依据或跨局学习；目前尚不保证每个人都有鲜明不同的表现。
NPC 上下文将公开发言顺序、翻牌身份、死亡时机与玩家说法分开提供；这些防护不代表模型推理不会出错。
...
正常终局后，[夜间复盘](docs/POSTGAME_REVIEW.md)展示已记录的行动目标与死因；
活局不公开，旧存档缺失的行动不作推测。模型提示强调立场连续性与阵营目标，
不强制投票策略，也不宣称已验证棋力提升。

开局顺序：**Agent 对话／网页 → 模型或已登记 Agent／主动离线 → 闯关／自由对局／继续 → 板子与人物**。
界面不等于智能驱动。网页以夜鸦主持的十二人顺时针圆桌呈现，手机也保留肖像；
点座位只是选择目标，还要在行动区确认，不会直接投票或用技能。

新玩家对局将警上与白天澄清分开：每位被问者合并回答本轮问题，并展示对应原句。
主持人明确收口及暂缓的追加问题，最后陈述不再开启追问。死亡技能结算后交接或撕毁警徽；
首夜死者和白天被放逐者可发遗言或跳过，其他死亡无遗言。这是本项目规则，并非所有狼人杀的通则。
旧存档保留原有遗言流程；完整规则可随时向主持人查询（`/rules`）。

网页可选择三十位预设人物或随机人物，名字、头像、性格与口头禅一起绑定，
每局仍是十二人：你选的人物与另外十一位不重复抽取的伙伴。可搜索名字或性格，
续存保留原班人马，不重新抽人。详见[人物库与素材说明](docs/CHARACTERS.md)。
与狼人／平民等秘密身份独立。自定义名字与人格设置仍保留。
每天放逐投票前，存活的真人都有一次可跳过的完整回应，不会因连续追问耗尽额度而失去解释机会。
离线模式通过情境选项表达，不理解任意自由文本。
澄清选项会对应提问者的公开记录；NPC 可以解释此前立场或保留判断，不把解释重复计成新的指控。

本机 Agent 可从网页的「添加本机 Agent 连接」开始：登记内置 Codex 适配器不会调用模型；
「检查连接」单独调用一次，只验证、不发牌。其他 Agent 通过可信本地包装器登记，
详见[模型连接](docs/MODEL_CONNECTIONS.md)。
...
从 30 个固定人格中抽取 12 人，支持替代人物、公共旁观与本地续存；不理解自由输入、不计闯关成绩。
使用方法及限制见[离线玩法](docs/OFFLINE_GAME.md)。

离线回应可关联原句，选择解释或保留判断，NPC 会作有上限的后续反馈。
30 名人物有明确的对话偏好，不是 30 个独立 AI 模型；仍不理解任意自然语言。
批量／消融验收见离线指南；未接入或要求安装 Laya。

离线讨论先提供几句情境回应与“先听听”，“其他说法”保留完整菜单。
离线与模型共用[查验声明对账](docs/CHECK_CLAIMS.md)，但不认证身份、不自动理解
任意自由文本。实验性推理辅助不代表已验证的实力或平衡性；细节与兼容说明见
[已知限制](docs/KNOWN_LIMITATIONS.md)和[更新日志](CHANGELOG.md)。
...
网页按「连接玩家 → 板子与身份 → 开始游戏」组织设置。入局后，十二座次
放在对话上方，个人身份与主持人规则单聊独立呈现。阅读旧发言时不会被
新消息强制拉到底部，可点击「最新发言」跟上进度；复盘关闭后仍可从牌桌
重新打开。布局兼容手机，界面支持中英文。
...
先看[支持与验证范围](docs/SUPPORT.md)和[近期路线图](docs/ROADMAP.md)。
反馈请注明版本、入口与驱动，不要附完整存档或凭据。CI 通过不等于玩法平衡或
真实模型表现已获证明。README 描述当前代码，不将规划当作已发布功能。
