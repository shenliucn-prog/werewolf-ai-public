...
engine without modifying v1 research sources. Actual run histories are local-only;
protocol failures must be reported separately from completed games.
...
python tests/audit_extreme.py outputs/extreme-run/condition-0.json
```
