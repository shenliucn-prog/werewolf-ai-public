...
Status: the [V3.1 continuation](research-runs/codex-shadow-day-v3-1-2026-09-05.md)
completed one bounded seven-seat episode, including revisions, ballots and shadow
reports. It reused seven validated initial drafts from V3 under explicitly recorded
lineage. Earlier failed runs remain intact. Completion demonstrates protocol wiring,
not optimal reasoning, judge accuracy, persuasion improvement or full-game results.
...
The original `shadow-day-v1` run is retained. `shadow-day-v2` separates
role and faction judgments and derives fixed-board constraints solely from each
owner's facts. Its [fresh live attempt](research-runs/codex-shadow-day-v2-2026-09-05.md)
also stopped at initial-table validation, this time because two actors put role
candidates into an unknown assertion field. Neither run completed a debate.
See the result report for the exact revised field contract and limitations.
...
The [V3 attempt](research-runs/codex-shadow-day-v3-2026-09-05.md) passed all initial
tables but rejected unsolicited tables in a challenge response before any leak.
...
python -m werewolf_web.research.codex_shadow_run --run-codex \
  --output docs/research-runs/codex-shadow-day-YYYY-MM-DD.json
```
