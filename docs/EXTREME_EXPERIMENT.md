...
The proposed budget is up to 440 calls across four games (110 safety cap each); games
...
python -m werewolf_web.research.extreme_run --run-codex --output-dir outputs/extreme-run
```
...
Actual run records and observations remain local and must not be committed.
