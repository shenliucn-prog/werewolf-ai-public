...
On 2026-09-04, the maintainer confirmed that the 12 NPC portraits were generated
with AI on their own local computer. The maintainer subsequently confirmed that
...
identifiers have been removed. A root `PUBLICATION_MANIFEST.json`, when present,
records exactly which files/fields changed and their before/after hashes. Do not
...
and localization code. In the publication-preparation follow-up, the maintainer
...
identifying metadata should be published. That separate privacy decision remains
in the [publication plan](PUBLICATION_PLAN.md). It does not change the portrait
