...
- A local Agent with command execution and a persistent interactive process can act as its relay. Start it unbuffered with `python -u -m werewolf_web.chat_game --lang en` after following the README setup and configuring a model connection. Use a PTY/session handle when the Agent's tools require one for ongoing input.
- This repository does **not** yet ship a universal Agent plugin, MCP server, installable host-specific integration. Compatibility with arbitrary Agent hosts has not been established. A chat-only Agent without local tools cannot run the game this way.
- Default NPC decisions use the configured API or Agent adapter, with a real preflight before identities are dealt. `--offline` is an explicitly chosen rule-flow test, not model gameplay. `--backend legacy` only rephrases local decisions. Never choose either silently. Model-player conjecture tables are not integrated yet; do not claim otherwise.
...
After process loss, use the saved game id with `--resume GAME_ID`; do not
inspect hidden checkpoint contents or invent a continuation. Read the repository's `AGENTS.md` for the complete relay obligations.
...
6. If the host cannot maintain the interactive process, say so. If it loses that process, try `--resume GAME_ID` using the recorded id. If recovery fails, explain the error and ask before starting over. Do not run concurrent interfaces against the same live game.
...
**尚无通用 Agent 插件、MCP 服务、各宿主的一键安装适配；未验证所有 Agent 产品。** 没有本地工具的纯聊天 Agent 无法按此方式运行。`--offline` 只关闭游戏自身的模型调用，不免除宿主 Agent 的使用成本。
...
- 让规则引擎判定结果，不编造进程没有产生的事件。保持同一交互进程；进程丢失时使用已记录的编号运行 `--resume GAME_ID`；恢复失败应说明错误并询问是否重开，不用模拟故事冒充恢复。
