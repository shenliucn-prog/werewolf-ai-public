...
vm.runInContext(fs.readFileSync(path.join(root, 'js', 'roundtable.js'), 'utf8'), dom.getInternalVMContext());
const flush = () => new Promise(resolve => setTimeout(resolve, 10));
(async () => {
  await flush();
  assert.equal(w.document.querySelector('.setup-grid > section').hidden, true);
  const entry = w.document.querySelector('#entryInterface');
  entry.value = 'web'; entry.dispatchEvent(new w.Event('change'));
  assert.equal(w.document.querySelector('.setup-grid > section').hidden, false);
  ...
  // Actual table controls select, never submit until the explicit confirmation.
  w.eval('renderSeats({seats:[{pos:1,name:"One",is_player:true,alive:true,character_id:"acheng"},{pos:2,name:"Two",alive:true,character_id:"amo"}]})');
  w.eval('showAction({kind:"vote",request_id:"seat-click",data:{candidates:[{pos:2,name:"Two"}]}})');
  const beforeSeat = requests.filter(r => r.url === '/api/action').length;
  w.document.querySelector('.seat[data-seat="2"]').click();
  assert.match(w.document.querySelector('#sendVote').textContent, /Two/);
  assert.equal(requests.filter(r => r.url === '/api/action').length, beforeSeat);
  w.eval('showAction({kind:"table_answer",request_id:"quoted",data:{from:"One",questions:[{from:"One",text:"The exact question?",event_no:9}]}})');
  assert.match(panel.textContent, /The exact question/);
  w.eval('showAction({kind:"table_answer",request_id:"offline-quoted",data:{questions:[{from:"One",text:"<b>Why?</b>",event_no:9}]},choices:[{id:"reply",group:"Respond",label:"Explain",suggested:true},{id:"skip",group:"Respond",label:"Skip",suggested:true}]})');
  assert.equal(panel.querySelector('blockquote').textContent, 'One · #9: <b>Why?</b>');
  assert.equal(panel.querySelector('blockquote b'), null);
  panel.querySelector('button').click(); await flush();
  assert.equal(requests.filter(r => r.url === '/api/action').pop().body.choice_id, 'reply');
  w.eval('showAction({kind:"table_answer",request_id:"farewell",data:{last_words:true,from:"Raven"}})');
  assert.match(panel.textContent, /遗言|last words/);
  w.eval('markSheriff(2); handleEvent({type:"badge",target:null,text:"Badge destroyed"})');
  assert.equal(w.document.querySelectorAll('.seat.sheriff').length, 0);
  // Agent interface prepares the selected brief without starting a Web game.
  entry.value = 'agent'; entry.dispatchEvent(new w.Event('change'));
  const beforeAgent = requests.length;
  w.document.querySelector('#newGame').click();
